		...
		<div class="form-group col-lg-4" style="display: inline-block; width: 600px; height: 280px;">
			<div class="card">
			  <div class="card-body" style="height: 280px;">
			    ...
			    <h6 class="card-subtitle mb-2 text-muted">좌석가격 : 금액 *인원 </h6>
...
<script>
	...
	$(function(){
   		$(".btn-check").on("click", function(){
   			var peopleNum = this.value; //이걸로 오더라인코드 수량 주면 될듯..? 사람 3명 => 오더라인 3개
   			  $.ajax({
   				type:"post",
   				url:"/cinema/people",
   				dataType : "text",
   				success: function(result){ $(".peopleNum").html(peopleNum); }
   			});
   		});	
   	});  
	...
	function chooseSeat() {
	    ...
	    //예매좌석을 확인하고 -> 상영관 예매좌석 표시(상영일정에서 isbooked된 자리 표시해주기 )
	    let booked = "";
	     ...
	     $(".btn-check").on("click", function() {
	    	 ...
	    	 //여기서 timeCode를 불러오고 싶음.
				/* $.ajax({
	   				...
	   				data:{ "timeCode" : ${time.timeCode} }, //서버에 보낼 parameter정보
